...
<div class="flex mx-auto my-2 {active} group  w-full">
    <button class="{disabled} w-full mx-auto max-w-md" on:click={toggleDropdown}>
        <div class="shadow-lg flex flex-1 flex-col min-h-full h-full w-full p-4 group group-[.is-inactive]:blur-sm group-[.is-active]:shadow-2xl focus-within:blur-none hover:shadow-md hover:border-primary-400 ease-in duration-100  text-left border-solid border-2 rounded-md border-primary-200">
            <h3 class="text-primary-600 underline group-hover:underline-offset-4 ease-in-out duration-300 forced-colors:block transition appearance-none">{title}</h3>
            <p class="text-primary-500 mb-3">{desc}</p>
            <div class="flex flex-col flex-1  mt-auto mb-0 min-h-fit my-auto relative w-full">
                <p class="text-primary-400 mt-auto mb-0 transparency:text-sm ">{tags_joined}</p>
            </div>
        </div>
